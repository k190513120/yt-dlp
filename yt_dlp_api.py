...
import tempfile
import json
# import whisper  # Removed to reduce image size
import io
import warnings
...
def transcribe_video():
    """转录视频音频为文字 - 当前不可用"""
    return jsonify({
        'error': 'Whisper功能暂时不可用，为了减少部署镜像大小已移除此功能',
        'message': 'Transcription feature is temporarily unavailable to reduce deployment image size'
    }), 503
